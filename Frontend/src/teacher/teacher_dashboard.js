...
export default function UI() {
  ...
  const nochapters = <div className='no_chapters'>No chapters have been added</div>
...
const newcourse_window=<div className='newcourse_parent'>
  <Dialog open={publishdialogopen} onClose={handleClose}>
        <DialogContent>
          <DialogContentText sx={{minHeight: '90px', minWidth:'400px', display:'flex', justifyContent:'center', alignItems:'center',fontSize:'16px'}}>
            { publishmessage==='Publish'?"Publishing":`${courseTitle} published successfully`}
          </DialogContentText>
        </DialogContent>
        {/* <DialogActions>
...
<div className='leftpanel'>
  <div className='newcourse_titlebox'>
    <div className='newcourse_title_container'>
      ...
      <p style={{ fontSize: '14px' ,textAlign:'left'}}>Completed Fields: {completedFields}/5</p>
    </div>
  </div>
...
const courselist_window=<div className="teacher_maincontent" style={{display:loading1&&loading2?'none':'grid'}}>
<div
...
<div className="newcoursebutton" style={{ display: !activeMenu ? "none" : "flex" }} onClick={()=>setwindowindex(1)}><p>{<AddCircleOutlineOutlinedIcon></AddCircleOutlineOutlinedIcon>}{" New Course"}</p></div>
...
const windows=[courselist_window,newcourse_window];
  ...
  return (
    <div className="teacher_dashboard_parent">
      ...
      {  windowindex!==0?
       <div className="exitbutton" onClick={() => setwindowindex(0)} style={{transition:'all 0s'}}>
       <p >EXIT</p>
     </div>:

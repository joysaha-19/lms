...
const register = asynchandler(async (req, res) => {
  ...
  const emailinuse = await Teachers.findOne({ email: email });
  if (emailinuse) {
    res.status(400).send("Email already in use");
    return 
  }
  const hashedpassword = await bcrypt.hash(password, 10);
  const added = await Teachers.create({
    username: username,
    email: email,
...
const login= asynchandler(async(req,res)=>{
  ...
  const usercheck=await Teachers.findOne({"email":email});
...
const getteacher = asynchandler(async (req, res) => {
  ...
  const teacher= await Teachers.findOne({username:req.query.username});
  res.json(teacher);
});

module.exports = { register, login, current, logout, update_profile,getteacher };